<#
.SYNOPSIS
Runs the source-pinned official MedNeXt-S-k3 pipeline without activating a shell environment.

.EXAMPLE
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\Code\scripts\run_mednext_cv_pipeline.ps1 -ConfirmRun

.EXAMPLE
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\Code\scripts\run_mednext_cv_pipeline.ps1 -SmokeTest -ConfirmRun

.EXAMPLE
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\Code\scripts\run_mednext_cv_pipeline.ps1 -ExperimentId <EXACT_ID> -Resume -ConfirmRun

.NOTES
The full default is Task501 folds 0..4, 100 epochs per fold, sequentially.
SmokeTest uses Task951 with exactly 8 real training and 2 real validation cases
from canonical fold 0, forces a stop after epoch 1, resumes to epoch 3, and then
completes the same evaluation/report lifecycle. No shell activation is required.
#>
param(
    [string]$ExperimentId = '',
    [switch]$Resume,
    ...
    [ValidateRange(1, 64)][int]$PreprocessingThreads = 8
)
...
if (-not $ConfirmRun) {
    throw (
        ...
        'MedNeXt protocol and available disk space.'
    )
}
...
if ($providedExperimentId) {
    ...
    $ExperimentId = New-MedNeXtExperimentId -Kind $experimentKind
}
...
try {
    Enter-ResearchGpuLocks -ExperimentId $ExperimentId -Backend mednext
    Assert-NoActiveResearchModelProcess
    Enable-ResearchSleepPrevention

    $bannerTitle = if ($SmokeTest) {
        "MEDNEXT REAL-DATA RESUME SMOKE TEST | $ExperimentId"
    }
    else {
        "OFFICIAL MEDNEXT-S-K3 100-EPOCH 5-FOLD CV | $ExperimentId"
    }
    Write-ResearchBanner -Title $bannerTitle -Color DarkCyan
    Write-Host "Python (direct, no activation): $script:ResearchPython"
    Write-Host "Task: $(if ($SmokeTest) { 'Task951_BraTS2023GLISmoke' } else { 'Task501_BraTS2023GLI' })"
    Write-Host "Experiment kind: $experimentKind"
    Write-Host "Folds: $($folds -join ', ')"
    Write-Host "Target epochs: $targetEpochs per fold"
    Write-Host 'Model: official MedNeXt-S, kernel 3; TTA OFF; native and canonical probabilities retained'
    ...
    if ([string]$experiment.experiment_id -ne $ExperimentId -or
        [string]$experiment.backend -ne 'mednext' -or
        [string]$experiment.experiment_kind -ne $experimentKind -or
        [string]$experiment.model_id -ne 'mednext_v1_s_kernel3') {
        throw "Experiment identity/kind mismatch in $experimentManifest"
    }
    foreach ($ownedInput in @(
        [pscustomobject]@{
            Name = 'MedNeXt model configuration'
            Path = [string]$experiment.model_config
            ExpectedHash = [string]$experiment.model_config_sha256
        },
        [pscustomobject]@{
            Name = 'canonical five-fold split'
            Path = [string]$experiment.split.source
    ...
    Write-ResearchStage -Number '2/12' -Name 'Pinned Environment, Official Source, and GPU System Check'
    Invoke-ResearchPython -Arguments @(
        '-m', 'glioma_seg.backends.mednext.backend',
        '--project-root', $script:ResearchProjectRoot,
        'system-check',
    ...
    Write-ResearchStage -Number '5/12' -Name 'Exact v1 Adapter, Official Preprocessing, and GPU Memory Gate'
    $memoryGateExperimentId = if ($SmokeTest) {
        $ExperimentId
    }
    else {
        "${ExperimentId}_memory_gate"
    }
    $smokePreprocessArguments = @(
        '-m', 'glioma_seg.backends.mednext.backend',
        '--project-root', $script:ResearchProjectRoot,
        'preprocess',
        '--experiment-id', $memoryGateExperimentId,
        '--smoke',
        '--threads', [string]$PreprocessingThreads,
        '--quick-train-cases', [string]$quickTrainCases,
        '--quick-validation-cases', [string]$quickValidationCases
    )
    Invoke-ResearchPython -Arguments $smokePreprocessArguments `
        -LogPath (Join-Path $logsDirectory 'preprocess_task951.log')
    if (-not $SmokeTest) {
        Invoke-ResearchPython -Arguments @(
            '-m', 'glioma_seg.backends.mednext.backend',
            '--project-root', $script:ResearchProjectRoot,
            ...
            '--quick-train-cases', [string]$quickTrainCases,
            '--quick-validation-cases', [string]$quickValidationCases
        ) -LogPath (Join-Path $logsDirectory 'preprocess_task501.log')
    }
    $preprocessingArtifacts = Join-Path $reportDirectory 'preprocessing_artifacts.json'
    $preprocessing = Get-Content -LiteralPath $preprocessingArtifacts -Raw | ConvertFrom-Json
    $expectedTaskName = if ($SmokeTest) {
        'Task951_BraTS2023GLISmoke'
    }
    else {
        'Task501_BraTS2023GLI'
    }
    $expectedPreprocessedCases = if ($SmokeTest) { 10 } else { 1251 }
    if (-not [bool]$preprocessing.valid -or
        [string]$preprocessing.task_name -ne $expectedTaskName -or
        [int]$preprocessing.case_count -ne $expectedPreprocessedCases -or
        (@($preprocessing.patch_size) -join ',') -ne '128,128,128' -or
        (@($preprocessing.target_spacing_mm) -join ',') -ne '1.0,1.0,1.0') {
        throw "Official MedNeXt preprocessing gate failed: $preprocessingArtifacts"
    }
    ...
    if ($Resume -and (Test-Path -LiteralPath $memoryOutput -PathType Leaf)) {
        try {
            ...
            $reuseMemoryGate = (
                ...
                (@($memoryEvidence.patch_size) -join ',') -eq '128,128,128' -and
                [bool]$memoryEvidence.official_loss_and_augmentation -and
                [double]$memoryEvidence.peak_reserved_mb -gt 0.0
            )
        }
        catch {
            $reuseMemoryGate = $false
    ...
    if ($reuseMemoryGate) {
        Write-Host 'Reused the valid experiment-local MedNeXt memory preflight.' -ForegroundColor Green
    }
    else {
        Invoke-ResearchPython -Arguments @(
            '-m', 'glioma_seg.backends.mednext.backend',
    ...
    if (-not $memoryIdentityOk) {
        throw "MedNeXt GPU memory evidence failed post-run verification: $memoryOutput"
    }
    if ([double]$memoryEvidence.peak_reserved_mb -gt [double]$memoryEvidence.total_vram_mb) {
        throw (
            'Official MedNeXt-S-k3 exceeded dedicated GPU memory during the real ' +
            'forward/loss/backward preflight: peak_reserved_mb=' +
            "$([double]$memoryEvidence.peak_reserved_mb), dedicated_total_mb=" +
            "$([double]$memoryEvidence.total_vram_mb). No full training was started."
        )
    }
    $memoryPreprocessingEvidence = Join-Path (
        Get-ResearchReportDirectory -ExperimentId $memoryGateExperimentId
    ) 'preprocessing_artifacts.json'
    if (-not (Test-Path -LiteralPath $memoryPreprocessingEvidence -PathType Leaf)) {
        throw "Memory-gate preprocessing evidence is missing: $memoryPreprocessingEvidence"
    }
    Write-ResearchJsonAtomic `
        -Path (Join-Path $reportDirectory 'memory_preflight_context.json') `
        -Value ([ordered]@{
            valid = $true
            primary_experiment_id = $ExperimentId
            preflight_experiment_id = $memoryGateExperimentId
            task_name = 'Task951_BraTS2023GLISmoke'
            source_fold = 0
            training_cases = 8
            validation_cases = 2
            target_epochs_for_smoke = 3
            model_id = [string]$experiment.model_id
            model_config_sha256 = [string]$experiment.model_config_sha256
            memory_preflight_json = [IO.Path]::GetFullPath($memoryOutput)
            memory_preflight_sha256 = (
                Get-FileHash -LiteralPath $memoryOutput -Algorithm SHA256
            ).Hash.ToLowerInvariant()
            preprocessing_artifacts_json = [IO.Path]::GetFullPath($memoryPreprocessingEvidence)
            preprocessing_artifacts_sha256 = (
                Get-FileHash -LiteralPath $memoryPreprocessingEvidence -Algorithm SHA256
            ).Hash.ToLowerInvariant()
            scope = (
                'One real forward, official loss, backward, and optimizer step using the ' +
                'unchanged MedNeXt-S-k3 128x128x128 plan on the deterministic real-data smoke task.'
            )
        })
    ...
    if ($PreflightOnly) {
        ...
        $resumeCommand = (
            ".\Code\scripts\run_mednext_cv_pipeline.ps1 " +
    ...
    Write-ResearchStage -Number '6/12' -Name 'Sequential Fold Training, Deep Audit, and Resume'
    $resultRoot = Get-MedNeXtResultDirectory -ExperimentId $ExperimentId
    foreach ($fold in $folds) {
        ...
        $audit = Invoke-MedNeXtFoldAudit -FoldDirectory $foldDirectory -OutputPath $auditPath
        if ([bool]$audit.Report.valid -and [bool]$audit.Report.complete) {
            Write-ResearchBanner -Title "FOLD $fold VERIFIED - REUSING COMPLETE ARTIFACTS" -Color Green
        ...
        if (Test-Path -LiteralPath $foldManifestPath -PathType Leaf) {
            ...
            if ([string]$foldManifest.experiment_id -ne $ExperimentId -or
                ...
                [int]$foldManifest.target_epochs -ne $targetEpochs -or
                [string]$foldManifest.model_id -ne 'mednext_v1_s_kernel3') {
        ...
        if ($SmokeTest) {
            if ([bool]$audit.Report.safe_to_resume) {
                $stoppedRuntimePath = Join-Path $foldDirectory 'runtime.json'
                if (-not (Test-Path -LiteralPath $stoppedRuntimePath -PathType Leaf)) {
                    throw 'Smoke resume was declared safe but its cumulative runtime evidence is missing.'
                }
                $stoppedRuntime = Get-Content -LiteralPath $stoppedRuntimePath -Raw |
                    ConvertFrom-Json
                $isForcedStopLeg = (
                    [bool]$stoppedRuntime.stopped_for_resume_test -and
                    [int]$stoppedRuntime.number_of_epochs -eq 1 -and
                    [int]$stoppedRuntime.target_epochs -eq 3
                )
                $isFinalArtifactRecovery = (
                    -not [bool]$stoppedRuntime.stopped_for_resume_test -and
                    [int]$stoppedRuntime.number_of_epochs -eq 3 -and
                    [int]$stoppedRuntime.target_epochs -eq 3
                )
                if (-not $isForcedStopLeg -and -not $isFinalArtifactRecovery) {
                    throw 'Smoke resume runtime is neither the epoch-1 forced stop nor epoch-3 recovery.'
                }
                $resumeLegTitle = if ($isForcedStopLeg) {
                    'SMOKE RESUME LEG - CONTINUING VERIFIED EPOCH-1 CHECKPOINT TO EPOCH 3'
                }
                else {
                    'SMOKE FINAL-ARTIFACT RECOVERY FROM VERIFIED EPOCH-3 CHECKPOINT'
                }
                Write-ResearchBanner `
                    -Title $resumeLegTitle `
                    -Color Yellow
                Invoke-ResearchPython -Arguments @(
                    '-m', 'glioma_seg.backends.mednext.backend',
                    '--project-root', $script:ResearchProjectRoot,
                ...
                if ($unexpected.Count -gt 0) {
                    throw (
                        'Fold 0 has non-resumable partial artifacts. Inspect the owned fold ' +
                        "directory and use a new experiment: $foldDirectory"
                    )
                }
                Write-ResearchBanner `
                ...
                $stoppedAudit = Invoke-MedNeXtFoldAudit `
            ...
            if (-not [bool]$audit.Report.safe_to_resume -and
                ...
                if ($unexpected.Count -gt 0) {
                    throw (
                        "Fold $fold has non-resumable partial artifacts. Inspect $foldDirectory " +
                        'and allocate a new experiment rather than overwriting evidence.'
                    )
                }
            }
            Write-ResearchBanner `
                -Title "TRAINING MEDNEXT FOLD $($fold + 1)/5 | 100 EPOCHS" `
                -Color DarkCyan
            $trainArguments = @(
                '-m', 'glioma_seg.backends.mednext.backend',
                '--project-root', $script:ResearchProjectRoot,
        ...
        $completedAudit = Invoke-MedNeXtFoldAudit `
            -FoldDirectory $foldDirectory `
            -OutputPath $auditPath
    ...
    if ($SmokeTest) {
        ...
        $evaluationArguments = @(
            ...
            '--artifact-manifest', (Join-Path $reportDirectory 'crossval_artifact_manifest.json'),
            '--output-dir', $reportDirectory,
    ...
    if ($Resume -and
        ...
        try {
            ...
            $failureStatisticsComplete = (
                ...
                [string]$failureStatistics.sources.metrics_per_case_sha256 -eq $currentMetricsHash -and
                [string]$failureStatistics.sources.crossval_integrity_sha256 -eq $currentIntegrityHash -and
    ...
    Write-MedNeXtTelemetryAggregation `
        -ExperimentId $ExperimentId `
        -ReportDirectory $reportDirectory `
        -Folds $folds `
    ...
    # Do not tee into the directory while the finalizer hashes all existing logs.
    Invoke-ResearchPython -Arguments $bundleArguments
    $reportManifest = Join-Path $reportDirectory 'report_manifest.json'
    if (-not (Test-Path -LiteralPath $reportManifest -PathType Leaf)) {
        throw 'Model report finalizer completed without report_manifest.json.'
    }
    $finalReceipt = Get-Content -LiteralPath $reportManifest -Raw | ConvertFrom-Json
    if ($SmokeTest -and [bool]$finalReceipt.is_final_baseline) {
        throw 'Smoke-test report was incorrectly labelled as a final baseline.'
    }
    if (-not $SmokeTest -and -not [bool]$finalReceipt.is_final_baseline) {
        throw 'The complete five-fold report was not labelled as a final baseline.'
    }

    $pipelineSucceeded = $true
    Write-ResearchBanner -Title 'MEDNEXT PIPELINE COMPLETED AND VERIFIED' -Color Green
